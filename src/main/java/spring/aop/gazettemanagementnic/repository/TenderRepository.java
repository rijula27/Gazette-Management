...
public interface TenderRepository extends JpaRepository<Tender, Long> {
    ...
    // List<Tender> findTendersOpenedAfter(@Param("cutoffDate") LocalDate
    // cutoffDate);
    ...
    @Query("SELECT t FROM Tender t WHERE t.announcement_Date <= :today AND (t.opening_Date >= :today OR t.opening_Date >= :oneMonthAgo) AND t.status.status_id = :statusId")
    List<Tender> findPublishedTenders(@Param("statusId") Long statusId, @Param("today") LocalDate today, @Param("oneMonthAgo") LocalDate oneMonthAgo);
