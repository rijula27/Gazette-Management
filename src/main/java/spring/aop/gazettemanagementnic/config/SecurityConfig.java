...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                .csrf(csrf -> csrf.ignoringRequestMatchers("/captcha-image")) // Enable CSRF for all endpoints except captcha-image
                ...
                .authorizeHttpRequests(authorize -> authorize
                        .requestMatchers("/login","/index","/","/home","/about","/functions","/gazette/display","/gazette/years/*/months","/gazette/years/*/months/*/dates","/gazette/years/*/months/*/dates/*","/tender/display","/tender/years/*/months","/tender/years/*/months/*","/tender","/tender/displayArchive","/contactUs","/organizationChart","/policies","/accessibilityStatement","/siteMap","/help","/accessibilityBrowsers","/test","/screenReader","/contact/display","/about/display","/gallery/images/**", "/captcha-image", "/css/**", "/js/**","/images/**").permitAll()
                        .requestMatchers("/gazette/pdf/**","/tender/pdf/**","/pdf/**").authenticated() // Require authentication for PDF viewing
                        ...
                        .requestMatchers("/admin","/admin_tender","/admin_template","/admin_creator_list","/admin/admin_display","/admin/admin_creator_list","/admin/admin_publisher_list","/admin/creator_upload","/admin/admin_delete/**","/admin/delete_creator/**","/admin/edit_creator","/admin/admin_tender_display","/upload_pdf","/admin/uploadPdf","/admin_pdf", "/admin/admin_pdf_display","/admin/pdf_delete/**","/admin_contactUs","/contact/save","/contact/edit","/contact/delete/**","/admin_aboutUs","/about/save","/about/aboutDisplay","/admin_gallery","/gallery/upload","/gallery/imageDisplay").hasAuthority("ADMIN")
                ...
                .formLogin(form -> form
                        ...
                        .failureHandler(authenticationFailureHandler)  // Custom failure handler for concurrent sessions
                        .permitAll()
                )
                ...
                .sessionManagement(session -> session
                        // Generate new session ID after login (prevents session fixation attacks)
                        .invalidSessionUrl("/login?session=expired")
                        .sessionConcurrency(concurrency -> concurrency
                                .sessionRegistry(sessionRegistry())  // Use SessionRegistry to track sessions
                                .maximumSessions(1)  // Only 1 active session per user
                                .maxSessionsPreventsLogin(true)  // Prevent new login if already logged in elsewhere
                                .expiredUrl("/login?session=expired")  // Redirect when forced logout
                        )
                );
